...
import CountdownTimer from "@/components/RemainingTimer";
...
const Posts: React.FC = () => {
  ...
  const fetchPosts = async () => {
    try {
      ...
      let timeToStart = data.rows.map((val: any) => (val.publish_time));
  ...
  const publishFn = async (slug: any) => {
    try {
      let adminToken = localStorage.getItem("adminToken")
      console.log("adminToken", adminToken)
      if (adminToken) {
        const { data } = await axios.put<any>(
          `${apiUrl}/admin/post/updatePostStatus/${slug}`,
          { headers: { Authorization: "Bearer " + adminToken } },
        );
      }
  ...
  return (
    <div className="mx-auto max-w-4xl p-6 pt-20">
      ...
      {posts.length === 0 && <div className="flex justify-center items-center space-x-3 text-center mt-30 m-auto">
        <svg className="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14" id="New-Folder--Streamline-Flex" height="14" width="14"><desc>New Folder Streamline Icon: https://streamlinehq.com</desc><g id="new-folder--empty-folder"><path id="Subtract" fill="#d7e0ff" d="M6.3308 1.68814c-0.13828 -0.3734 -0.4849 -0.63009 -0.88267 -0.64819C4.01145 0.974562 3.14465 0.992082 1.9919 1.07324c-0.48162 0.03391 -0.86456 0.41351 -0.91967 0.89316C0.73486 4.90279 0.622932 7.84939 0.974376 10.7925c0.119254 0.9987 0.937034 1.7246 1.915354 1.8311 2.76247 0.3008 5.48523 0.2931 8.24757 -0.0076 0.9599 -0.1045 1.7305 -0.8647 1.845 -1.8235 0.2172 -1.81895 0.2751 -3.68351 0.1399 -5.97063 -0.056 -0.94753 -0.799 -1.71004 -1.745 -1.78836 -1.3445 -0.1113 -2.27895 -0.11768 -3.93138 -0.15264 -0.40948 -0.00867 -0.77231 -0.26734 -0.91455 -0.65142l-0.20047 -0.54131Z" stroke-width="1"></path><path id="Subtract_2" stroke="#4147d5" stroke-linecap="round" stroke-linejoin="round" d="M6.33085 1.68814c-0.13829 -0.3734 -0.4849 -0.63009 -0.88267 -0.64819C4.01149 0.974562 3.1447 0.992082 1.99195 1.07324c-0.48162 0.03391 -0.86456 0.41351 -0.91967 0.89316C0.734905 4.90279 0.622978 7.84939 0.974421 10.7925c0.119259 0.9987 0.937029 1.7246 1.915349 1.8311 2.76248 0.3008 5.48524 0.2931 8.24753 -0.0076 0.96 -0.1045 1.7306 -0.8647 1.8451 -1.8235 0.2172 -1.81895 0.2751 -3.68351 0.1399 -5.97063 -0.0561 -0.94753 -0.7991 -1.71004 -1.745 -1.78836 -1.3445 -0.1113 -2.279 -0.11768 -3.93143 -0.15264 -0.40948 -0.00867 -0.77232 -0.26734 -0.91456 -0.65142l-0.20046 -0.54131Z" stroke-width="1"></path></g></svg><span>No Posts yet.</span></div>}
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {posts &&
          posts.map((post, index) => (
            ...
            >
              <CountdownTimer publishTime={post.publish_time} onPublish={publishFn} postId={post.id} slug={post.slug} key={index} />
