...
const CommentsModeration = () => {
    ...
    const fetchComments = () => {
        ...
        axios.get(`${apiUrl}/admin/post/getComments`, {
            headers: {
    ...
    const handleApprove = async (commentId) => {
        try {
            const adminToken = localStorage.getItem('adminToken');
            let { data } = await axios.post(`${apiUrl}/admin/post/comments/approve/${commentId}`, {}, {
                headers: {
                    Authorization: `Bearer ${adminToken}`
                }
            });
    ...
    const handleDisapprove = async (commentId) => {
        try {
            const adminToken = localStorage.getItem('adminToken');
            let { data } = await axios.post(`${apiUrl}/admin/post/comments/disapprove/${commentId}`, {}, {
                headers: {
                    Authorization: `Bearer ${adminToken}`
                }
            });
    ...
    const deleteComment = async (commentId) => {
        try {
            const adminToken = localStorage.getItem('adminToken');
            let { data } = await axios.delete(`${apiUrl}/admin/post/comments/deletecomment/${commentId}`, {
                headers: {
                    Authorization: `Bearer ${adminToken}`
                }
            });
    ...
    return (
        <div className='pt-20 p-5'>
            <h2 className='text-center my-5 text-2xl font-semibold text-gray-700'>Comments Moderation</h2>
            <div className='overflow-x-auto shadow-md rounded-lg'>
                <table className='w-full border-collapse bg-white text-left'>
                    <thead>
                        <tr className='bg-gray-100 text-gray-600 uppercase text-sm leading-normal'>
                            <th className='py-3 px-6 border-b'>Blog Title</th>
                            <th className='py-3 px-6 border-b'>Username</th> {/* New column for Username */}
                    ...
                    <tbody className='text-gray-700 text-sm font-light'>
                        {commentsData.map((comment) => (
                            <tr key={comment.commentid} className='border-b hover:bg-gray-50'>
                                ...
                                <td className='py-3 px-6 border-b'>{comment.username}</td> {/* Displaying the username */}
                                <td className='py-3 px-6 border-b'>{comment.comment}</td>
                                <td className='py-3 px-6 border-b'>
                                    <span className={`py-1 px-3 rounded-full text-xs font-semibold ${comment.approve === 0 ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
