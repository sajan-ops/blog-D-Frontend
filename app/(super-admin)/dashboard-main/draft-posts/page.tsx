...
const Posts: React.FC = () => {
  ...
  const handleDelete = async (slug: number) => {
    try {
      ...
      let { data } = await axios.delete(`${apiUrl}/admin/post/delete-post/${slug}`, {
        headers: { Authorization: `Bearer ${adminToken}` },
      }); // Adjust the URL based on your API
  ...
  const handlePublish = async (slug: any) => {
    try {
      setIsPublishing(true);
      const adminToken = localStorage.getItem("adminToken");

      let { data } = await axios.put(`${apiUrl}/admin/post/makeStatusPublish/${slug}`, {}, {
        headers: {
        ...
        {posts &&
          posts.map((post) => (
            <div className="relative rounded-lg border border-gray-200 bg-white p-4 shadow-md transition-shadow duration-200 hover:shadow-lg">
              ...
              >
                <Send size={16} className={`${isPublishing ? 'animate-pulse' : ''}`} />
